...
def google_callback():
    try:
        # state 검증 수동으로 처리 (Railway 세션 문제 우회)
        ...
        app.logger.error(f'Google callback error: {err_detail}')
        # MismatchingStateError면 자동 재시도
        if 'MismatchingState' in str(type(e).__name__) or 'state' in str(e).lower():
